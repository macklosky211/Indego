...
@export var Idle_Interaction_Spots : Array[Marker3D]
...
func get_all_creatures() -> Array[Creature_Controller]:
	...
	for child : Node3D  in Map_Collider.get_overlapping_bodies():
...
func get_idle_interaction_spots() -> Array[Marker3D]: return Idle_Interaction_Spots

func object_entered(body : Node3D) -> void:
	if body is State_Controller: body.current_area = self
