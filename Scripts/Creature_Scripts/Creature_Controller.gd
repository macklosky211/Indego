...
## Not sure if this variable should exist or not.
var health : float = 100.0:
	set(value):
		if value < 0: monster_is_dead.emit()
		health = value
...
## This variable represents how long this creature is stunned for.
...
#	if fmod(current_frame, frame_delay) != 0: return

	if creature_state is Object and creature_state.has_method("_update"): creature_state._update(self, delta)
